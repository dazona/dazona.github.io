...
QUnit.test("Transaction date order", function(assert) {
  var t = makeTree();
  var txLunch = {
    timestamp: toMillis("2/2/15", "12:00"),
    description: "lunch",
    amount: 320,
    debit: "expenses",
    credit: "front",
  ...
  var txDinner = {
    timestamp: toMillis("2/2/15", "19:00"),
    description: "dinner",
    amount: 120,
    debit: "expenses",
    credit: "front",
  ...
  assert.ok(txLunch.timestamp < txDinner.timestamp);

  // 7 hour difference
  assert.equal(txDinner.timestamp - txLunch.timestamp, 7 * 60 * 60 * 1000);
});
