...
QUnit.test("Reverse transaction", function(assert) {
  ...
  var originalTx = { timestamp: 0, description: "pass go", amount: 20000, debit: "bank", credit: "open" };

  t.recordTransaction(originalTx);
  assert.ok(!originalTx.reversed);
  
  var reverseTx = reverseTransaction(originalTx);
  t.recordTransaction(reverseTx);

  assert.equal(t.accounts['bank'].balance, 0);
  assert.equal(originalTx.reversed, true);
});
